...
export default function App() {
  ...
  const [prompts, setPrompts] = useState<Prompt[]>(() => {
    ...
    const visiblePromptIds = new Set(legacyPrompts.map(prompt => prompt.id));
  ...
  const moveCategoryByOffset = (categoryId: string, offset: number) => {
    const currentIndex = categories.findIndex(category => category.id === categoryId);
    const targetIndex = currentIndex + offset;

    if (currentIndex === -1 || targetIndex < 0 || targetIndex >= categories.length) return;

    moveCategory(categoryId, categories[targetIndex].id);
  };
  ...
  const movePromptByOffset = (promptId: string, offset: number) => {
    const visiblePromptIds = filteredPrompts.map(prompt => prompt.id);
    const currentVisibleIndex = visiblePromptIds.findIndex(id => id === promptId);
    const targetVisibleIndex = currentVisibleIndex + offset;

    if (
      currentVisibleIndex === -1 ||
      targetVisibleIndex < 0 ||
      targetVisibleIndex >= visiblePromptIds.length
    ) return;

    movePrompt(promptId, visiblePromptIds[targetVisibleIndex]);
  };
  ...
  }, [prompts, searchQuery, activeCategory, isRecycleBinView]);

  return (
    <div className="flex flex-col h-screen bg-[#F9F9F9] text-[#1A1A1A] font-sans overflow-hidden">
      <Toaster position="top-right" />
      
      {/* Desktop Title Bar (Simulated) */}
      <div data-tauri-drag-region className="h-8 bg-white border-b border-[#E5E5E5] flex items-center justify-between px-4 select-none drag-region">
        <div className="flex items-center gap-2">
          {/* Spacer for native macOS traffic lights */}
          <div className="w-14" />
          <span className="text-[11px] font-medium text-[#666]">OurPrompt</span>
        </div>
        <div className="flex items-center gap-4">
          <Badge variant="outline" className="h-4 text-[9px] px-1.5 border-[#E5E5E5] text-[#A1A1A1] font-bold uppercase">v1.0.0 Stable</Badge>
        </div>
      </div>

      <div className="flex flex-1 min-h-0 overflow-hidden">
        {/* Sidebar */}
        <aside className="w-64 flex-shrink-0 border-r border-[#E5E5E5] bg-white flex flex-col min-h-0">
            ...
            <div className="space-y-6 py-2">
              <div className="space-y-1">
                <div className="flex items-center justify-between px-3 mb-2">
                  <h2 className="text-[10px] font-bold text-[#A1A1A1] uppercase tracking-widest">{t.categoryLibrary}</h2>
                  <button onClick={addCategory} className="text-[#A1A1A1] hover:text-[#1A1A1A] transition-colors">
                ...
                {categories.map((cat) => {
                  ...
                  const categoryLabel = cat.id === 'other' ? t.recycleBin : cat.name;
                  const categoryIndex = categories.findIndex(category => category.id === cat.id);
                  const canMoveCategoryUp = categoryIndex > 0;
                  const canMoveCategoryDown = categoryIndex < categories.length - 1;
                  return (
                    <div
                      key={cat.id}
                    ...
                    >
                      ...
                      <button
                        ...
                        className={`w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium transition-all ${
                          activeCategory === cat.id ? 'bg-[#F0F0F0] text-[#1A1A1A]' : 'text-[#666] hover:bg-[#F5F5F5]'
                        } ${
                      ...
                      {canManageCategory && (
                        <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all">
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              moveCategoryByOffset(cat.id, -1);
                            }}
                            disabled={!canMoveCategoryUp}
                            title={t.moveUp}
                            className="p-1 text-[#A1A1A1] hover:text-[#1A1A1A] transition-all disabled:cursor-not-allowed disabled:opacity-40"
                          >
                          ...
                          <button
                            ...
                            className="p-1 text-[#A1A1A1] hover:text-[#1A1A1A] transition-all"
                          >
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium transition-all ${
                    activeCategory === 'all' ? 'bg-[#F0F0F0] text-[#1A1A1A]' : 'text-[#666] hover:bg-[#F5F5F5]'
                  }`}
                >
              ...
              <div className="space-y-1">
                <h2 className="px-3 text-[10px] font-bold text-[#A1A1A1] uppercase tracking-widest mb-2">{t.system}</h2>
                <button
                  onClick={() => setIsSettingsOpen(true)}
                  className="w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium text-[#666] hover:bg-[#F5F5F5]"
                >
          ...
          <div className="p-4 border-t border-[#E5E5E5]">
            <Button 
              onClick={() => openEditor()} 
              className="w-full bg-[#1A1A1A] hover:bg-[#333] text-white rounded-lg h-10 shadow-sm"
            >
        ...
        {/* Main Content */}
        <main className="flex-1 flex flex-col min-w-0 min-h-0 bg-[#F9F9F9]">
          {/* Top Bar */}
          <header className="h-16 border-b border-[#E5E5E5] bg-white flex items-center justify-between px-8 flex-shrink-0">
            <div className="flex items-center gap-4 flex-1 max-w-xl">
              <div className="relative w-full">
                ...
                <Input 
                  ...
                  className="pl-10 h-9 bg-[#F5F5F5] border-none focus-visible:ring-1 focus-visible:ring-[#1A1A1A]"
                />
              </div>
            </div>
            
            <div className="flex items-center gap-2 ml-4">
              <div className="flex items-center bg-[#F0F0F0] rounded-md p-1">
                <button 
                  onClick={() => setViewMode('grid')}
                  className={`p-1.5 rounded ${viewMode === 'grid' ? 'bg-white shadow-sm' : 'text-[#666]'}`}
                >
                  <LayoutGrid className="w-4 h-4" />
                </button>
                <button 
                  onClick={() => setViewMode('list')}
                  className={`p-1.5 rounded ${viewMode === 'list' ? 'bg-white shadow-sm' : 'text-[#666]'}`}
                >
            ...
            <div className="p-8">
              ...
              {isRecycleBinView ? (
                ...
                >
                  <AnimatePresence mode="popLayout">
                    ...
                    const promptIndex = filteredPrompts.findIndex(item => item.id === prompt.id);
                    const canMovePromptUp = promptIndex > 0;
                    const canMovePromptDown = promptIndex < filteredPrompts.length - 1;
                    return (
                    <motion.div
                      key={prompt.id}
                    ...
                    >
                      ...
                      {viewMode === 'grid' ? (
                        <Card
                          ...
                          className={`group border-[#E5E5E5] hover:border-[#1A1A1A] transition-all shadow-none bg-white h-64 flex flex-col overflow-hidden ${dragOverPromptId === prompt.id ? 'ring-2 ring-[#DADADA]' : ''} ${draggedPromptId === prompt.id ? 'shadow-xl shadow-black/10 cursor-grabbing' : ''} ${draggedPromptId !== prompt.id ? 'cursor-pointer' : ''}`}
                        >
                          <CardHeader className="p-5 pb-2">
                            ...
                            <CardTitle className="text-base font-bold mt-2 line-clamp-1">{prompt.title}</CardTitle>
                          </CardHeader>
                          <CardContent className="px-5 py-2 flex-1 overflow-hidden">
                            ...
                            <p className="text-sm text-[#666] line-clamp-4 leading-relaxed font-mono text-[13px]">
                              {prompt.content}
                            </p>
                          </CardContent>
                          <CardFooter className="px-5 py-4 bg-[#FAFAFA] border-t border-[#E5E5E5] flex justify-between items-center opacity-0 group-hover:opacity-100 transition-opacity">
                            <div className="flex gap-1">
                              <Button
                                variant="ghost"
                                size="icon"
                                onClick={() => movePromptByOffset(prompt.id, -1)}
                                disabled={!canMovePromptUp}
                                title={t.moveUp}
                                className="h-8 w-8 hover:bg-white disabled:cursor-not-allowed"
                              >
                                <ChevronUp className="w-3.5 h-3.5" />
                              </Button>
                              <Button
                              ...
                              <Button variant="ghost" size="icon" onClick={() => requestDeletePrompt(prompt)} className="h-8 w-8 text-red-500 hover:text-red-600 hover:bg-white">
                                <Trash2 className="w-3.5 h-3.5" />
                              </Button>
                            </div>
                            <div className="flex gap-2">
                              <Button 
                                variant="outline" 
                                size="sm" 
                                className="h-8 text-[11px] font-bold border-[#E5E5E5] bg-white"
                                onClick={() => {
                        ...
                        <div
                          onDoubleClick={() => openPromptViewer(prompt)}
                          className={`group flex items-center justify-between p-4 bg-white border border-[#E5E5E5] rounded-lg hover:border-[#1A1A1A] transition-all ${dragOverPromptId === prompt.id ? 'ring-2 ring-[#DADADA]' : ''} ${draggedPromptId === prompt.id ? 'shadow-xl shadow-black/10 cursor-grabbing' : ''} ${draggedPromptId !== prompt.id ? 'cursor-pointer' : ''}`}
                        >
                          <div className="flex items-center gap-4 min-w-0">
                            ...
                            <div className="min-w-0">
                              <h3 className="text-sm font-bold truncate">{prompt.title}</h3>
                              <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-[#B0B0B0] mt-1">{t.doubleClickHint}</p>
                              <p className="text-xs text-[#666] truncate mt-0.5">{prompt.content}</p>
                            </div>
                          </div>
              ...
              {!isRecycleBinView && filteredPrompts.length === 0 && (
                <div className="flex flex-col items-center justify-center py-32 text-center">
                  ...
                  <p className="text-sm text-[#666] mt-1">{t.noPromptHint}</p>
                </div>
              )}
              ...
              {isRecycleBinView && filteredDeletedCategories.length === 0 && filteredDeletedPrompts.length === 0 && (
                <div className="flex flex-col items-center justify-center py-20 text-center">
                  ...
                  <p className="text-sm text-[#666] mt-1">{t.recycleBinHint}</p>
                </div>
              )}
            </div>
          </ScrollArea>
        ...
        <DialogContent className="sm:max-w-[420px] p-0 overflow-hidden border-none shadow-2xl rounded-xl">
          <div className="p-6">
            <DialogHeader className="mb-5">
              ...
              <DialogDescription>{t.languageSettingsDesc}</DialogDescription>
            </DialogHeader>
        ...
        <DialogContent className="sm:max-w-[420px] p-0 overflow-hidden border-none shadow-2xl rounded-xl">
          <div className="p-6">
            <DialogHeader className="mb-3">
              ...
              <DialogDescription>
                {t.deleteCategoryDesc(categoryToDelete?.name)}
              </DialogDescription>
            </DialogHeader>
          </div>
          <DialogFooter className="border-[#E5E5E5] bg-[#FAFAFA]">
        ...
        <DialogContent className="sm:max-w-[420px] p-0 overflow-hidden border-none shadow-2xl rounded-xl">
          <div className="p-6">
            <DialogHeader className="mb-5">
              ...
              <DialogDescription>{t.renameCategoryDesc(categoryToRename?.name)}</DialogDescription>
            </DialogHeader>
            <div className="space-y-2">
              <label className="text-xs font-bold text-[#666] uppercase tracking-wider">{t.categoryName}</label>
              <Input
      ...
      >
        <DialogContent className="sm:max-w-[760px] p-0 overflow-hidden border-none shadow-2xl rounded-[22px]">
          {viewingPrompt && (
            <>
              <div className="border-b border-[#ECE8DE] bg-[linear-gradient(135deg,_#FCFBF8_0%,_#F5F1E8_100%)] px-7 py-6">
                <DialogHeader className="gap-3">
                  <div className="flex items-center gap-2 flex-wrap">
                    <Badge variant="secondary" className="bg-white text-[#666] text-[10px] px-2 py-0.5 h-6 font-bold uppercase shadow-sm">
                      {categories.find(c => c.id === viewingPrompt.category)?.name ?? t.category}
                    </Badge>
                    <Badge variant="secondary" className={`text-[10px] px-2 py-0.5 h-6 font-bold uppercase ${viewingPrompt.isDefault ? 'bg-[#F0F0F0] text-[#666]' : 'bg-[#EEF4EA] text-[#4E6B4E]'}`}>
                      {viewingPrompt.isDefault ? t.builtin : t.custom}
                    </Badge>
                    <Badge variant="outline" className="border-[#E6DFD0] bg-transparent text-[10px] px-2 py-0.5 h-6 font-bold uppercase tracking-[0.18em] text-[#9B8A67]">
                      {t.viewPrompt}
                    </Badge>
                  </div>
                  <DialogTitle className="text-2xl font-bold tracking-tight text-[#1A1A1A]">
                    {viewingPrompt.title}
                  </DialogTitle>
                  <DialogDescription className="text-[14px] leading-6 text-[#6D6558]">
              ...
              <div className="px-7 py-6">
                <div className="mb-3 flex items-center justify-between gap-3">
                  <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-[#A1A1A1]">
                    {t.promptContent}
                  </p>
                ...
                <div className="max-h-[420px] overflow-y-auto rounded-[18px] border border-[#E8E5DD] bg-[#FBFBFA] p-5">
                  <pre className="whitespace-pre-wrap break-words font-mono text-[13px] leading-7 text-[#333]">
              ...
              <DialogFooter className="border-[#E5E5E5] bg-[#FAFAFA]">
                <Button
                  ...
                  className="font-bold text-xs border-[#E5E5E5] bg-white"
                >
