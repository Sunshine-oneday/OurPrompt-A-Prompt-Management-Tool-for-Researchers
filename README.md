...
下载并运行 `OurPrompt-v1.0.0-windows-x64-setup.exe` 即可。

### macOS

下载并安装 `OurPrompt-v1.0.0-macos-universal.dmg` 后，首次运行前执行：
...
下载 `OurPrompt-v1.0.0-linux-x64.AppImage`，赋予执行权限后运行：

```bash
chmod +x OurPrompt-v1.0.0-linux-x64.AppImage
./OurPrompt-v1.0.0-linux-x64.AppImage
```
